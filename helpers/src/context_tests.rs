...
fn context_basic_usage() {
    ...
    // Should match exact format: "src/context_tests.rs:line"
    assert!(result.starts_with("src/context_tests.rs:"));
...
fn context_with_anyhow() {
    ...
    assert!(error_string.contains("context_tests.rs"));
}
...
fn context_closure_reuse() {
    ...
    assert!(ctx1.starts_with("src/context_tests.rs:"));
